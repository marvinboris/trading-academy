<div class="form-group {{ $class }}">
    <div class="input-group">
        ...
        @switch($type)
            @case('textarea')
            <textarea id="{{ $id }}" class="form-control text-fa border-0 bg-black-10 py-4 px-3 @error($name) border is-invalid @enderror" name="{{ $name }}" placeholder="{{ __($placeholder) }}" {{ $required ?? 'required' }} autocomplete="{{ $name }}">{{ old($name) }}</textarea>
                @break
            @case('select')
